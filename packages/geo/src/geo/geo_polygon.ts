...
export const roundPolygon = (polygon: IPoint[], radius: number) => {
  ...
  for (let i = 0; i < polygon.length; i++) {
    ...
    const r = getCorrectedRadius(
      i === 0 ? p1 : pointMid(p1, p2),
      p2,
      i === polygon.length - 1 ? p3 : pointMid(p2, p3),
      radius,
    );
