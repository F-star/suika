import { type IPathCommand, type IPoint } from '../type';
...
export const arcToCubic = ({
  ...
  if (angleDir) {
    pathData.push({ type: 'M', points: [p1] });
    pathData.push({ type: 'C', points: [p2, p3, p4] });
  } else {
    pathData.push({ type: 'M', points: [p4] });
    pathData.push({ type: 'C', points: [p3, p2, p1] });
  }
